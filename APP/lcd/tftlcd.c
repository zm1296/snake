...
void LCD_ShowChar(char ch,int x,int y,int size,u16 fg_color,u16 bg_color)
{
  int px=0,py=0,y0=y;
	...
	for(px=0;px<csize;px++)
	{
		...
		{
			case 12:
					temp=(char)ascii_1206[ch][px];
				break;
			case 16:
					temp=(char)ascii_1608[ch][px];
				break;
			case 24:
					temp=(char)ascii_2412[ch][px];
				break;
		}
		...
		for(py=0;py<8;py++)
		{
			 if(temp &(1<<(7-py)))
			 {
				   LCD_DrawFRONT_COLOR(x,y,fg_color);
			 }
			 else
			 {
				   LCD_DrawFRONT_COLOR(x,y,bg_color);
			 }
			 y++;
			 if(y-y0==size)
			 {
...
void LCD_ShowChina(char *str,int x,int y,u16 fg_color,u16 bg_color)
{
	  ...
	  for(WordNum=0;WordNum<30;WordNum++)
  	{
			  ...
			  if(CnChar32x29[WordNum].Index[0]==*str && CnChar32x29[WordNum].Index[1]==*(str+1))
				{
					   pos=(char*)CnChar32x29[WordNum].Msk;
					   for(i=0;i<24;i++)
					   {
							 ch1=*pos++;
							 ch2=*pos++;
							 ...
							 for(j=0;j<8;j++)
							 {
								 if(ch1 & (1<<(7-j)))
								 {
									   LCD_DrawFRONT_COLOR(x+i,y+j,fg_color);
								 }
								 else
								 {
									   LCD_DrawFRONT_COLOR(x+i,y+j,bg_color);
								 }
								 if(ch2 & (1<<(7-j)))
								 {
									   LCD_DrawFRONT_COLOR(x+i,y+j+8,fg_color);
								 }
								 else
								 {
									   LCD_DrawFRONT_COLOR(x+i,y+j+8,bg_color);
								 }
								 if(ch3 & (1<<(7-j)))
								 {
									   LCD_DrawFRONT_COLOR(x+i,y+j+16,fg_color);
								 }
								 else
								 {
									   LCD_DrawFRONT_COLOR(x+i,y+j+16,bg_color);
								 }
							 } 
					   }   
				}
...
void LCD_ShowString(char *str,int x,int y,int size,u16 fg_color,u16  bg_color)
{
	 ...
	 for(i=0;i<lenth;i++)
	 {
	    LCD_ShowChar(*str,x,y,size,fg_color,bg_color);
		str++;
		x+=11;
	 }
}
void LCD_ShowChinaStr(char *str,int x,int y,u16 fg_color,u16  bg_color)
{
	   ...
	   for(i=0;i<lenth;i++)
	   {
	       LCD_ShowChina(pos,x,y,fg_color,bg_color);
		   pos+=2;
		   x+=24;
	   }
...
void LCD_ShowFontHZ16(u16 x, u16 y, u8 *cn,u16 color1)
{
	...
	while (*cn != '\0')
	{
		for (wordNum=0; wordNum<100; wordNum++)//若数组内字体数量大于30时 ，需更改数值
		{	//wordNum扫描字库
			if ((CnChar16x16[wordNum].Index[0]==*cn)&&(CnChar16x16[wordNum].Index[1]==*(cn+1)))
			{
				...
				{	//MSK的位数
					...
					for(j=0;j<8;j++) 
					{
						if((color&0x80)==0x80)
						{
							LCD_DrawFRONT_COLOR(x,y,color1);//FRONT_COLOR
						}else
						{
						//LCD_DrawFRONT_COLOR(x,y,color);//BACK_COLOR
						} 
						color<<=1;
						x++;
						if((x-x0)==16)
